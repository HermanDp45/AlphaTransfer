#!/usr/bin/env python3
"""Regression tests for the independent quant/macro research pipeline."""
...
quant = load_module("quant_feature_ablation_tested", SCRIPT_DIR / "quant_feature_ablation.py")
fetcher = load_module("fetch_open_data_tested", SCRIPT_DIR / "fetch_open_data.py")
identity = load_module("cbr_cross_rate_identity_tested", SCRIPT_DIR / "cbr_cross_rate_identity.py")
...
class AvailabilityTest(unittest.TestCase):
    ...
    def test_primary_real_panel_has_no_negative_source_age(self) -> None:
        ...
        panel = quant.build_feature_panel(REPO_ROOT, SCRIPT_DIR / "normalized", core, quant.PROFILES["primary"])
...
class AdaptiveCadenceTest(unittest.TestCase):
    ...
    def test_final_market_portfolio_is_subset_of_corridor_policy_signals(self) -> None:
        core = quant.load_core(REPO_ROOT)
        panel = core.add_target(core.build_panel(REPO_ROOT / "data" / "cbr_daily.csv"), 5)
...
class DataIntegrityTest(unittest.TestCase):
    ...
    def test_real_nbk_boundary_corruption_is_absent(self) -> None:
        frame = pd.read_csv(SCRIPT_DIR / "normalized" / "project_nbk_fx_snapshot.csv")
    ...
    def test_all_artifact_hashes_and_pipeline_hash_match_manifest(self) -> None:
        manifest = json.loads((SCRIPT_DIR / "data_manifest.json").read_text())
        ...
        self.assertEqual(
            quant.sha256(SCRIPT_DIR / "fetch_open_data.py"),
    ...
    def test_post_2024_kzt_cross_rate_identity_is_reproduced(self) -> None:
        observations = pd.read_csv(
            REPO_ROOT / "data" / "kzt_v0" / "observations.csv",
            parse_dates=["effective_date"],
        )

        audit = identity.historical_kzt_identity(observations)

        self.assertGreaterEqual(len(audit), 400)
        self.assertGreater(audit["exact_to_cbr_rounding"].mean(), 0.99)
        self.assertTrue((audit["nbk_rate_date"] < audit["effective_date"]).all())

    def test_live_five_corridor_cross_rate_identity_is_exact(self) -> None:
        audit = identity.live_five_corridor_identity()

        self.assertEqual(set(audit["corridor"]), {"AMD", "KGS", "KZT", "TJS", "UZS"})
        self.assertTrue(audit["exact_to_four_decimals"].all())
