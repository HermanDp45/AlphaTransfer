...
def build_features(sources:Mapping[str,pd.DataFrame],as_of:str|pd.Timestamp,*,corridor='KZT')->pd.DataFrame:
    ...
    if isinstance(as_of,str) and re.fullmatch(r'\d{4}-\d{2}-\d{2}',as_of) is None:raise ValueError('as_of must be a calendar date YYYY-MM-DD, not an intraday timestamp')
    end=pd.Timestamp(as_of)
    if end.tz is not None or end!=end.normalize():raise ValueError('as_of must be a naive calendar date; batch time is fixed at10:05 Moscow')
