...
| --- | --- | --- | --- |
| Сигнал лучше случайного дня | отдельные события H3 и H5; базовая вероятность считается внутри каждого года и валютного направления | итоговый коэффициент превосходства 1,504 для H3 и 1,986 для H5; во всех шести годовых срезах он выше 1,30 | все периоды уже использовались в исследовании |
...
| Коэффициент превосходства (lift) | число успехов, делённое на ожидаемое число успехов при случайном выборе дней | во сколько раз сигнал информативнее случайного дня |
...
При объединении нескольких лет коэффициент превосходства не усредняется. Для каждого года и валютного направления базовая вероятность умножается на число сигналов, после чего фактическое число успехов делится на сумму таких ожиданий. Это сохраняет корректное сравнение со случайным выбором дня при изменении рыночного режима.
...
| Коэффициент превосходства | **1,504×** | **1,986×** |
...
H3 обеспечивает регулярную коммуникацию на коротком горизонте. H5 снижает среднюю частоту на 41% и отбирает более редкие даты, которые на его собственном целевом событии дают коэффициент превосходства 1,986 и преимущество 95,6 базисного пункта. Различие целей и базовых вероятностей не позволяет объяснять этот разрыв одним только горизонтом.

### Устойчивость по годам

![Устойчивость по годам](assets/model-metrics/02_annual_stability.png)

| Год | Модель | Базовая вероятность | Точность | Коэффициент превосходства | Недельное покрытие | Сигналов в неделю | Преимущество, б.п. | Упущенная выгода, б.п. | Ошибка Брайера | ROC AUC |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
Во всех шести годовых срезах коэффициент превосходства выше 1,30, а преимущество момента положительно. H3 во всех годах обеспечивает недельное покрытие не ниже 80%. Частота H5 находится в заранее установленном годовом диапазоне от 0,45 до 0,85 сигнала в неделю, а её итоговое значение входит в целевой диапазон от 0,60 до 0,70.
...
Для H3 строгий порог 0,5 повышает итоговый коэффициент превосходства до 1,698, но в худшем году оставляет сигналы лишь в 37,3% недель. Выбранное правило даёт коэффициент 1,504 и покрытие 85,9%. Для H5 выбранная частота составляет 0,659 сигнала в неделю при минимальном годовом коэффициенте превосходства 1,711. Таким образом, правила задают две осмысленные точки на границе между качеством и частотой.
...
| Период | Обучение на последних 120 месяцах: коэффициент / сигналов в неделю / преимущество | Обучение на истории с 2010 года: коэффициент / сигналов в неделю / преимущество |
| --- | ---: | ---: |
...
| Период | Обучение на последних 120 месяцах: коэффициент / покрытие / преимущество | Обучение на истории с 2010 года: коэффициент / покрытие / преимущество |
| --- | ---: | ---: |
...
Обучение на последних 120 месяцах обеспечивает более высокое общее покрытие, но в 2026 году коэффициент превосходства равен 1,235 и не достигает порога 1,30. Полная история проходит требования по качеству, выгоде и покрытию во всех трёх годах. В 2025 году её результат слабее, поэтому выбор основан на устойчивости худшего года, а не на превосходстве в каждом отдельном периоде.

| Разность «полная история минус 120 месяцев» | Оценка | 95%-й доверительный интервал по месячным блокам |
| --- | ---: | ---: |
| Коэффициент превосходства | +0,051× | [−0,088; +0,172] |
...
| Сравнение на 2023–2025 годах | Коэффициент превосходства | Преимущество момента | Ошибка Брайера | Недельное покрытие |
| --- | ---: | ---: | ---: | ---: |
...
На H3 календарь повысил коэффициент превосходства с 1,021 до 1,066 и преимущество момента с 4,4 до 10,2 базисного пункта, но слегка ухудшил ошибку Брайера с 0,2523 до 0,2531. На H5 коэффициент снизился с 1,071 до 1,055, а ошибка выросла с 0,2397 до 0,2422, хотя преимущество момента увеличилось с 9,9 до 12,2 базисного пункта. На H20 коэффициент снизился с 1,042 до 0,953. Устойчивого улучшения на нескольких горизонтах не обнаружено, поэтому календарные поля не вошли в итоговый набор.
...
| Базовая вероятность / коэффициент превосходства | 49,8% / **1,351×** | базовая вероятность рассчитана по 647 вневыборочным дням |
...
| Год | Основных сигналов | Уточнений | Точность | Коэффициент превосходства | Повышение к концу окна |
| --- | ---: | ---: | ---: | ---: | ---: |
| 2024 | 52 | 15 | 66,7% | 1,339× | +80,5 б.п. |
| 2025 | 63 | 24 | 54,2% | 1,259× | +43,2 б.п. |
| 2026* | 36 | 22 | 81,8% | 1,361× | +132,4 б.п. |

Результат неоднороден: в 2025 году коэффициент равен 1,259 и не достигает исследовательской границы 1,30. Поэтому сила формулировки и порог дополнительной модели требуют проверки на новом периоде. Её целевое событие тесно связано с основным H3 и не служит независимым подтверждением качества основной модели.

## 10. Выбор архитектуры и конфигурации

### Принцип выбора

Сравнение охватывало не только алгоритм, но и всё правило принятия решения: целевое событие, длину истории, состав признаков, числовую обработку, настройку вероятностей и ограничение частоты. Это необходимо, поскольку высокая площадь под ROC-кривой без приемлемой частоты не решает задачу коммуникации, а высокий коэффициент превосходства на нескольких десятках дат может оставлять клиента без сигналов большую часть года.

Вариант допускался к итоговому сравнению, если в каждом году его коэффициент превосходства был не ниже 1,30, преимущество момента оставалось положительным, а частота соответствовала назначению режима. Затем последовательно сравнивались результат в худшем году, общий коэффициент превосходства, преимущество момента и ошибка Брайера. Выбирать задним числом отдельную модель для каждого проверочного года не разрешалось.

Первичный поиск охватывал все горизонты из задания: один, три, пять, десять и двадцать дат действия курса. В раннем сопоставимом опыте с классическими моделями лучшие коэффициенты превосходства составляли для них 1,110; 1,055; 1,066; 1,089 и 1,107. Минимальный годовой результат ни у одного варианта не превышал 1,084. После расширения данных устойчивые и различимые с продуктовой точки зрения решения были получены для горизонтов в три и пять дат. Одного дня недостаточно для доставки сообщения и действия клиента, а десять и двадцать дней слабее соответствуют формулировке «выгодно сейчас» и повышают цену ожидания.
...
Архитектура и начальное состояние генератора случайных чисел были зафиксированы до итогового сравнения длины истории. В ранней проверке разных запусков коэффициент превосходства менялся от 1,432 до 1,692, а недельное покрытие — от 65,1% до 80,9%. Лучший запуск задним числом не выбирался. Такой порядок уменьшает смещение отбора, но показывает, что в следующем цикле необходимо оценивать разброс результатов между повторными обучениями.

### Отдельная модель KZT

На одинаковом отрезке 2023–2025 годов локальная TabM с 33 признаками дала на KZT коэффициент превосходства 1,584, ошибку Брайера 0,1843 и преимущество 61,2 базисного пункта. Модель, обученная сразу на пяти коридорах, при оценке на KZT дала 1,410, 0,1985 и 49,1 базисного пункта соответственно.
...
| Модель на 33 признаках, 2023–2025 годы | Коэффициент превосходства | Точность | Недельное покрытие | Ошибка Брайера | Преимущество момента |
| --- | ---: | ---: | ---: | ---: | ---: |
...
| Предыдущая версия V3 с прежним правилом частоты, KZT | коэффициент 1,478 и покрытие 88,2% | сильный ориентир, однако итоговые конфигурации дают более высокие оценки качества; системы различаются не только архитектурой |
...
| Коэффициент превосходства на своей цели | 1,504× | 1,986× | обе модели информативнее случайного дня |
