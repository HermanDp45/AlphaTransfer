...
def main():
 global OUT
 parser=argparse.ArgumentParser();parser.add_argument('--now-predictions',default=str(SPRINT/'selected_predictions.csv.gz'));parser.add_argument('--now-history');parser.add_argument('--selection',default=str(SPRINT/'selection.json'));parser.add_argument('--output-dir',default=str(OUT));args=parser.parse_args();OUT=Path(args.output_dir).resolve();assert OUT.is_relative_to(HERE),'Outputs must remain inside product research folder';OUT.mkdir(exist_ok=True)
 ...
 source_sha={};snapshots={};(OUT/'input_snapshot').mkdir(exist_ok=True)
 for position,path in enumerate([Path(args.selection),nowpath,*sources]):
  data=path.read_bytes();dest=OUT/'input_snapshot'/f'{position}_{path.name}';dest.write_bytes(data);key=str(path.relative_to(ROOT));source_sha[key]=c.sha(dest);snapshots[key]={'path':str(dest.relative_to(HERE)),'sha256':c.sha(dest)}
 assert json.loads((HERE/snapshots[str(Path(args.selection).relative_to(ROOT))]['path']).read_text())==selection,'Root selection changed during snapshot; rerun'
 now=pd.read_csv(HERE/snapshots[str(nowpath.relative_to(ROOT))]['path'],parse_dates=['date']);now=now[now.config_id.eq(config)&now.policy.eq(policyname)]
 nh=pd.concat([pd.read_csv(HERE/snapshots[str(p.relative_to(ROOT))]['path'],parse_dates=['date']) for p in sources],ignore_index=True);nh=nh[nh.config_id.eq(config)].copy();assert len(nh)>0
 ...
 c.save(OUT/'manifest.json',{'now_selection':selection,'now_input_sha256':source_sha,'preserved_input_snapshots':snapshots,'now_history_sources':[str(p.relative_to(ROOT)) for p in sources],'code_sha256':c.sha(Path(__file__)),'protocol_sha256':c.sha(HERE/'JOINT_PROTOCOL.md'),'semantic_routing_addendum_sha256':c.sha(HERE/'ROUTING_ADDENDUM.md'),'closing_predictions_sha256':c.sha(c.OUT/'closing_predictions.csv.gz'),'closing_history_sha256':c.sha(c.OUT/'closing_history.csv.gz'),'prediction_sha256':c.sha(OUT/'predictions.csv.gz'),'selection_uses_2026':selection.get('selection_uses_2026',True),'joint_test_threshold_search':False,'metric_contract':'separate NOW and CLOSING hits/lift; only cadence and reference forward utility pooled'})
