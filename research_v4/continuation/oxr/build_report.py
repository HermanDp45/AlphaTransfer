...
def main():
    ...
    ci=pd.read_csv(HERE/'paired_intervals.csv')
    winner=json.loads((HERE/'selection.json').read_text())['primary_candidate']
    names=['v3_120m','oxr_basis_120m_delay24h','oxr_basis_120m_delay48h','oxr_returns_120m_delay24h','oxr_full_120m_delay24h','oxr_coverage_120m']
    primary=s[s.scope.eq('all')&s.track.eq('development_2023_2025')&s.config_id.isin(names)]
    primary=primary.set_index('config_id').loc[names].reset_index()
    tests=s[s.scope.eq('all')&s.config_id.isin(['v3_120m',winner])&~s.track.eq('development_2023_2025')]
    history=s[s.scope.eq('all')&s.track.isin(['development_2023_2025','test2026_january_freeze'])&s.config_id.isin(['oxr_basis_120m_delay24h','oxr_basis_120m_since2020','oxr_basis_120m_since2022'])]
    intervals=ci[ci.scope.eq('all')&ci.config_id.eq(winner)&ci.block.eq('month')]
    body=f'''# OXR: проверка пользы имеющейся истории с 2018 года
...
{table(primary,['config_id','brier','signals','lift','forward_delta_bps'])}
...
{table(tests,['track','config_id','brier','signals','lift','forward_delta_bps'])}

{table(intervals,['track','delta_brier','ci_low','ci_high','blocks'])}
...
{table(history,['track','config_id','brier','signals','forward_delta_bps'])}
